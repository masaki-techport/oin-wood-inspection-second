"""
Sequential Presentation Processor (migrated from analysis directory).

This module preserves the original presentation image processing functionality
as a fallback for when parallel processing is not available or fails.
"""
...
logger = logging.getLogger('BaslerCamera.SequentialPresentationProcessor')

class SequentialPresentationProcessor:
    """Handles sequential presentation image processing for display and UI (fallback)"""
        ...
        try:
            ...
            with self.camera.db_handler.Session() as session:
                ...
                try:
                    ...
                    group_count = total_images  # Support unlimited groups
                    ...
                    if group_count == 1:
                        ...
                        # Support unlimited groups with Excel-style labeling (A, B, ..., Z, AA, AB, etc.)
                        ...
                        # Assign group names using Excel-style labeling (A, B, ..., Z, AA, AB, etc.)
                        group_names = [to_label(i + 1) for i in range(group_count)]
                    ...
                    # Normalize paths and validate files
                    valid_groups = set()
