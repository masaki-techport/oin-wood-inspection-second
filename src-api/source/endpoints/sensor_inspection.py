...
try:
    ...
    print(
        "[WARNING] Basler camera not available in sensor_inspection - pypylon not installed"
    )
    BaslerCamera = None
...
from streaming.sensor_sse import (
    sensor_broadcaster,
...
SENSOR_SIMULATION_MODE = app_config.getboolean(
            ...
            if config:
                ...
                if "buffer" in config:
                    if "duration_seconds" in config["buffer"]:
                        BUFFER_DURATION_SECONDS = config["buffer"][
                            "duration_seconds"
                        ]
        ...
        def dispatch_async():
            try:
                ...
                loop.run_until_complete(
                    broadcast_sensor_event(
                        "inspection-clear", clear_event_data
                    )
                )
                loop.close()
            except Exception as e:
...
def _clear_ui_immediately():
    ...
    try:
        ...
        if current_camera and hasattr(
            current_camera, "last_inspection_results"
        ):
        ...
        if (
            current_camera
            and hasattr(current_camera, "buffer_manager")
            and current_camera.buffer_manager
        ):
            if (
                hasattr(
                    current_camera.buffer_manager, "temp_section_assembler"
                )
                and current_camera.buffer_manager.temp_section_assembler
            ):
                current_camera.buffer_manager.temp_section_assembler.reset()
                cleared_components.append("temp_sections")
                info_print(
                    "✅ Cleared temp sections (inspection results from A-E frames)"
                )

        # 3. Clear sensor capture data (affects UI state)
        if sensor_capture and hasattr(
        ...
        info_print(
            f"⚡ IMMEDIATE clear completed in {elapsed_time:.1f}ms: {', '.join(cleared_components)}"
        )
    ...
    try:
        ...
        def clear_buffer_manager_data():
            ...
            if (
                ...
                if (
                    hasattr(buffer_manager, "results_storage")
                    and buffer_manager.results_storage
                ):
                ...
                if (
                    hasattr(buffer_manager, "analysis_queue")
                    and buffer_manager.analysis_queue
                ):
                ...
                # Clear result cache
                if (
                    hasattr(buffer_manager, "result_cache")
                    and buffer_manager.result_cache
                ):
        ...
        elapsed_time = (time.time() - start_time) * 1000
        info_print(
            f"🔄 Background clear completed in {elapsed_time:.1f}ms: {', '.join(cleared_components)}"
        )
...
def _clear_all_ui_components():
    ...
    try:
        ...
        background_thread = threading.Thread(
            target=_clear_background_data, daemon=True
        )
        background_thread.start()

        info_print(
            f"🎉 UI clearing initiated - immediate: {immediate_time:.1f}ms, background: async"
        )
...
async def start_sensor_inspection(request: StartInspectionRequest):
    ...
    try:
        info_print(
            f"Starting sensor inspection with camera type: {camera_type}, AI threshold: {ai_threshold}%"
        )
        ...
        try:
            memory_image_cache.clear_all()
            info_print(
                "Cleared in-memory image preview cache for new inspection"
            )
        except Exception as cache_err:
        ...
        debug_print(
            f"Getting camera of type {camera_type} from camera manager"
        )
        try:
            current_camera = camera_manager.get_camera(
                camera_type, "sensor_inspection"
            )
            camera_created = True
            ...
            if camera_connected:
                info_print(
                    f"{camera_type} camera connected successfully via camera manager"
                )
            else:
                debug_print(
                    f"{camera_type} camera connection failed via camera manager"
                )

        except Exception as e:
            error_msg = str(e)
            info_print(
                f"Error getting {camera_type} camera from camera manager: {e}"
            )
            ...
            if "exclusively opened by another client" in error_msg:
                camera_in_use = True
                info_print(
                    "CAMERA IN USE ERROR: The camera is currently being used by another application"
                )
                info_print(
                    "Please close any other applications that might be using the camera (like Pylon Viewer)"
                )
        ...
        if not camera_connected:
            info_print(
                f"Warning: {camera_type} camera connection failed, continuing in simulation-only mode"
            )

        # Reset temporary sections for a clean start of new inspection
        try:
            if (
                hasattr(current_camera, "buffer_manager")
                and current_camera.buffer_manager
                and getattr(
                    current_camera.buffer_manager,
                    "temp_section_assembler",
                    None,
                )
            ):
                current_camera.buffer_manager.temp_section_assembler.reset()
                info_print(
                    "TempSectionAssembler reset for new inspection start"
                )
        ...
        try:
            debug_print(
                f"Initializing SensorTriggeredCapture with {camera_type} camera"
            )
            ...
            if camera_type == "basler" and camera_connected:
                ...
                debug_print(
                    "Configuring BaslerCamera for sensor-triggered recording"
                )
                try:
                    # Clear any previous inspection results when starting new session
                    current_camera.last_inspection_results = None
                    debug_print(
                        "Cleared previous inspection results from camera"
                    )
                    ...
                    current_camera.buffer_size = int(
                        BUFFER_DURATION_SECONDS * BUFFER_FPS
                    )
                    ...
                    info_print(
                        f"Successfully configured BaslerCamera with AI threshold {ai_threshold}%"
                    )
                except Exception as config_error:
                    debug_print(
                        f"Error configuring BaslerCamera: {config_error}"
                    )
                ...
                try:
                    ...
                    current_camera.last_inspection_results = None
                    debug_print(
                        "Cleared previous inspection results from disconnected camera"
                    )

                    debug_print(
                        f"Setting AI threshold to {ai_threshold}% on disconnected BaslerCamera"
                    )
                    current_camera.set_ai_threshold(ai_threshold)
                except Exception as config_error:
                    debug_print(
                        f"Error setting AI threshold on disconnected BaslerCamera: {config_error}"
                    )

            # For other camera types, set AI threshold if method exists
            if camera_type != "basler" and hasattr(
                current_camera, "set_ai_threshold"
            ):
                try:
                    # Clear any previous inspection results when starting new session
                    if hasattr(current_camera, "last_inspection_results"):
                        current_camera.last_inspection_results = None
                        debug_print(
                            "Cleared previous inspection results from non-basler camera"
                        )

                    debug_print(
                        f"Setting AI threshold to {ai_threshold}% on {camera_type} camera"
                    )
                    current_camera.set_ai_threshold(ai_threshold)
                except Exception as config_error:
                    debug_print(
                        f"Error setting AI threshold on {camera_type} camera: {config_error}"
                    )

            # Create sensor capture system
            debug_print(
                f"Creating SensorTriggeredCapture with camera_interface: {current_camera is not None}"
            )
            ...
            info_print(
                f"Error initializing SensorTriggeredCapture: {capture_error}"
            )
            if DEBUG_MODE:
                traceback.print_exc()
        ...
        try:
            debug_print(
                f"Initializing SensorMonitor (simulation_mode={SENSOR_SIMULATION_MODE})"
            )
            # Force simulation mode if camera is not connected
            effective_simulation_mode = (
                SENSOR_SIMULATION_MODE or not camera_connected
            )
            if not camera_connected and not SENSOR_SIMULATION_MODE:
                debug_print(
                    "Forcing simulation mode because camera is not connected"
                )

            sensor_monitor = SensorMonitor(
                simulation_mode=effective_simulation_mode
            )
        ...
        try:
            debug_print(
                f"Starting sensor monitoring. sensor_capture: {sensor_capture is not None}"
            )
            if sensor_capture is None:
                raise ValueError(
                    "sensor_capture is None. Cannot start monitoring."
                )
            ...
            def combined_sensor_callback(result: Optional[str], state: str):
                ...
                try:
                    ...
                    if result == "CLEAR_RESULTS":
                        ...
                        info_print(
                            f"🔵 CLEAR_RESULTS with state={state} - IMMEDIATE UI clearing"
                        )
                        immediate_time = _clear_ui_immediately()
                        ...
                        info_print(
                            f"🔵 CLEAR_RESULTS completed in {immediate_time:.1f}ms - UI cleared immediately"
                        )
                        ...
                        info_print(
                            f"🔵 Starting capture and analysis for state={state}"
                        )
                        sensor_capture.handle_sensor_decision(None, state)

                        info_print(
                            f"🔵 CLEAR_RESULTS with state={state} - capture and analysis started"
                        )
                    else:
                        # Handle normal sensor decision (original logic)
                        info_print(
                            f"🔵 SENSOR CALLBACK: result={result}, state={state}"
                        )
                    ...
                    if result == "pass_L_to_R":
                        info_print(
                            "Sensor detected PASS_L_TO_R - starting camera capture"
                        )
                        if hasattr(
                            ...
                            info_print(
                                "Warning: sensor_capture does not have start_sensor_triggered_capture method"
                            )
            ...
            debug_print(
                "Prepared sensor capture system - waiting for sensor triggers"
            )
        ...
        status_message = (
            f"Sensor-based inspection started with {camera_type} camera."
        )
        if not camera_connected:
            if camera_in_use:
                ...
                status_message += (
                    " Camera not connected, running in simulation mode only."
                )
        ...
        start_response = {
            ...
            "simulation_mode": (
                sensor_monitor.simulation_mode if sensor_monitor else True
            ),
...
async def clear_sensor_flag():
    ...
    try:
        if sensor_monitor and hasattr(sensor_monitor, "status_tracker"):
            ...
            return {
                "status": "error",
                "message": "Sensor monitor not available",
            }
    except Exception as e:
...
async def clear_all_ui_components():
    ...
    try:
        info_print(
            "🧹 API: Clearing all UI components with immediate + background approach"
        )
...
async def get_sensor_inspection_status():
    ...
    try:
        if not sensor_monitor or not sensor_capture:
            debug_print(
                "[STATUS] Sensor inspection not active - no monitor or capture"
            )
            inactive_status = {
                "active": False,
        ...
        last_update_diff = (
            round((current_time - sensor_status["last_update_time"]) * 1000, 2)
            if "last_update_time" in sensor_status
            else None
        )
        debug_print(
            f"[STATUS] Active - Camera: {current_camera_type}, Sensors: A={sensor_status['sensor_a']}, B={sensor_status['sensor_b']}, "
        ...
        if not clear_requested:
            if (
                not inspection_data
                and current_camera
                and hasattr(current_camera, "last_inspection_results")
            ):
            ...
            inspection_data = None
            debug_print(
                "[STATUS] Clear requested - setting inspection_data to None"
            )
        ...
        if (
            ...
            and is_forward_pass
            and inspection_data
            and inspection_data.get("inspection_id")
        ):
            try:
                ...
                with SessionLocal() as db_session:
                    ...
                    if inspection_result:
                        ...
                        debug_print(
                            f"[STATUS] Fetched fresh inspection results from database for ID {inspection_data['inspection_id']}"
                        )
                    else:
                        debug_print(
                            f"[STATUS] No inspection results found in database for ID {inspection_data['inspection_id']}"
                        )
            except Exception as e:
                error_print(
                    f"[STATUS] Error fetching fresh inspection results: {e}"
                )
        elif clear_requested:
            debug_print(
                "[STATUS] Clear requested - skipping database fetch for inspection results"
            )
        elif not is_forward_pass:
            debug_print(
                f"[STATUS] Non-forward result '{last_result}' - skipping database fetch for inspection results"
            )

        # Debug logging for inspection data
        if inspection_data:
            debug_print(
                f"[STATUS] Including inspection data in response: inspection_id={inspection_data.get('inspection_id')}"
            )
            ...
            if not sensor_status.get("clear_requested", False):
                if (
                    "inspection_details" not in inspection_data
                    and hasattr(current_camera, "last_inspection_results")
                    and current_camera.last_inspection_results
                ):
                    if (
                        "inspection_details"
                        in current_camera.last_inspection_results
                    ):
                        inspection_data["inspection_details"] = (
                            current_camera.last_inspection_results[
                                "inspection_details"
                            ]
                        )
                        debug_print(
                            f"[STATUS] Preserved inspection_details from previous data"
                        )
        else:
        ...
        if clear_requested or not is_forward_pass:
            ...
            if clear_requested:
                debug_print(
                    "[STATUS] Clear requested - setting all inspection data to null"
                )
            else:
                debug_print(
                    f"[STATUS] Non-forward result '{last_result}' - clearing inspection data/results for UI"
                )
        ...
        status_response = {
            ...
            "capture": {
                ...
                "processing_active": capture_status.get(
                    "processing_active", False
                ),
                "sensors_active": capture_status.get("sensors_active", False),
                "sensor_a": sensor_status[
                    "sensor_a"
                ],  # Add sensor states directly to capture status
                "sensor_b": sensor_status[
                    "sensor_b"
                ],  # for easier frontend access
                ...
                "buffer_status": {
                    ...
                    "max_buffer_size": camera_status.get(
                        "max_buffer_size", BUFFER_DURATION_SECONDS * BUFFER_FPS
                    ),
                },
            },
...
async def trigger_test_sequence():
    ...
    if not sensor_monitor:
        return JSONResponse(
            status_code=400, content={"error": "Sensor inspection not active"}
        )
    ...
    if not sensor_monitor.simulation_mode:
        return JSONResponse(
            status_code=400,
            content={
                "error": "Test sequences only available in simulation mode"
            },
        )
...
def toggle_sensor_a():
    ...
    if not sensor_monitor.simulation_mode:
        ...
        return JSONResponse(
            status_code=400,
            content={
                "error": "Manual control only available in simulation mode"
            },
        )
...
def update_sensor_configuration(
    ...
    if buffer_duration is not None:
        BUFFER_DURATION_SECONDS = max(
            5, min(300, buffer_duration)
        )  # Limit between 5s and 5min
    ...
    try:
        ...
        abs_inspection_dir = os.path.join(
            root_dir, "data", "images", "inspection"
        )
        config = {
            "dio": {
                "device_name": "DIO000",
...
def set_ai_threshold(ai_threshold: int):
    ...
    if not current_camera:
        return JSONResponse(
            status_code=400, content={"error": "No active camera"}
        )
    ...
    try:
        ...
        if hasattr(current_camera, "set_ai_threshold"):
            ...
            return JSONResponse(
                status_code=400,
                content={
                    "error": "Camera does not support AI threshold setting"
                },
            )
...
def get_inspection_result(
    inspection_id: int, session: Session = Depends(get_session)
):
    """
    ...
    try:
        info_print(
            f"[INSPECTION_RESULT] Fetching results for inspection_id: {inspection_id}"
        )
        ...
        if not inspection_result:
            info_print(
                f"[INSPECTION_RESULT] No results found for inspection_id: {inspection_id}"
            )
            return JSONResponse(
                status_code=404,
                content={
        ...
        info_print(
            f"[INSPECTION_RESULT] Successfully retrieved results for inspection_id: {inspection_id}"
        )
...
def debug_camera_status():
    ...
    debug_info = {
        ...
        "has_last_inspection_results": hasattr(
            current_camera, "last_inspection_results"
        ),
        "last_inspection_results": getattr(
            current_camera, "last_inspection_results", None
        ),
...
def debug_presentation_images(inspection_id: int):
    ...
    try:
        ...
        with SessionLocal() as session:
            presentations = (
                session.query(InspectionPresentation)
                .filter(InspectionPresentation.inspection_id == inspection_id)
                .order_by(InspectionPresentation.group_name)
                .all()
            )
            ...
            for p in presentations:
                image_info = {
                    ...
                    "file_exists": (
                        os.path.exists(p.image_path) if p.image_path else False
                    ),
                }
                debug_info["presentation_images"].append(image_info)
                debug_info["file_existence"][p.group_name] = image_info[
                    "file_exists"
                ]
...
def get_latest_frame():
    ...
    try:
        ...
        if (
            current_camera_type == "basler"
            or current_camera_type == "basler_legacy"
        ):
